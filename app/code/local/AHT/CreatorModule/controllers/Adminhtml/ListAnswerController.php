...
class AHT_CreatorModule_Adminhtml_CreatorModuleController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Item Manager'), Mage::helper('adminhtml')->__('Answer Manager'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				$model->save();
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('creatormodule')->__('Item was successfully saved'));
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($creatormoduleIds)) {
            ...
            try {
                foreach ($creatormoduleIds as $creatormoduleId) {
                    $creatormodule = Mage::getSingleton('creatormodule/creatormodule')
                        ->load($creatormoduleId)
                        ->setStatus($this->getRequest()->getParam('status'))
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($creatormoduleIds))
                );
            } catch (Exception $e) {
